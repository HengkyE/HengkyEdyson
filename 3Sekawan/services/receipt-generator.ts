...
export async function shareReceiptAsPDF(
  ...
  try {
    ...
    const html =
      type === "cash"
        ? generateCashReceiptHTML(data)
        : generateWholesaleReceiptHTML(data, format, showWatermark);

    // Cash receipts use 80mm width for thermal printer format
    // Wholesale uses A4 (210mm) or A6 (80mm) based on format
    const width = type === "cash" ? 80 : format === "A4" ? 210 : 80;
    // A4 height is 297mm
    ...
    if (Platform.OS === "web") {
      ...
      if (format === "A4") {
        ...
        // Add padding to simulate print margins
      ...
      try {
        ...
        const canvas = await html2canvas(element, {
          // A4 PDFs can get very large; keep scale reasonable to avoid Telegram upload issues
          scale: format === "A4" ? 1.25 : 2,
          useCORS: true,
          logging: false,
      ...
      // Native: expo-print width/height are points (pt), not mm.
      // Convert from mm so A4 and 80mm render with the expected layout.
      console.log("Creating PDF file on native using expo-print...");
      const nativeWidth = Math.round(width * MM_TO_PT);
      const nativeHeight = height ? Math.round(height * MM_TO_PT) : undefined;
...
export async function generatePDFFromHTML(html: string, widthMm: number = 80): Promise<string> {
  try {
    ...
    if (Platform.OS === "web") {
      ...
      // On native, use expo-print with pt units.
      // For A4-like widths, force standard A4 height.
      console.log("Creating PDF file on native using expo-print...");
      const nativeWidth = Math.round(widthMm * MM_TO_PT);
      const nativeHeight = widthMm >= 200 ? Math.round(297 * MM_TO_PT) : undefined;
