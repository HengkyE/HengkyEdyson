...
export async function getNextGrosirInvoiceNo(): Promise<number> {
  ...
  if (!existingData) {
    // Create initial system data with 'notaGrosir' ID (matching old system)
    const { data, error } = await supabase
      .from("systemData")
...
> {
  ...
  if (error) {
    // Don't silently return [] because that looks like "all users have profiles".
    if (
      error.code === "42883" ||
      error.code === "PGRST202" ||
      error.code === "404" ||
      ...
      throw new Error(
        "Database function get_users_without_profiles is not installed. Please run the latest Supabase migration first."
      );
    }
    throw error;
  }
...
export async function createJualanKontan(
  ...
  try {
    ...
    // Generate ID using timestamp (matching old system behavior)
    console.log("Creating jualanKontan with ID:", id);
    ...
    console.log("JualanKontan created successfully:", data);

    // Create sale items if provided
    if (items.length > 0) {
      try {
        await createJualanItems(items, id, undefined);
        console.log("Sale items created successfully for jualanKontan:", id);
      } catch (itemsError) {
        console.error("Error creating sale items, but sale was created:", itemsError);
        // Don't throw - sale was created successfully, items are secondary
      }
    }
...
export async function getJualanKontanByDateRange(
  ...
  // Filter by userId if provided (for cashiers)
  if (userId) {
...
/**
 * Get grosir payments within a date range (by payment received date)
 */
...
export async function createJualanGrosir(
  ...
  // Prepare initial payment history if setorGrosir > 0
...
export async function createJualanItems(
  ...
  try {
    ...
    // Ensure any "manual" items have a corresponding `barangs` row so FK constraints don't block checkout.
...
export async function addGrosirPayment(
  ...
  try {
    ...
    // Get current sale and all payments for this invoice to recompute totals
...
export async function createGrosirDraft(
  ...
  // Ensure any manual items have a `barangs` row (FK safety for draft items if enforced).
  await ensureBarangsExistForCartItems(items);
