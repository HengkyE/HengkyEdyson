...
import type { PaymentReceiptData, ReceiptData } from "@/edysonpos/utils/receipt-formatter";
import { NativeModules, Platform } from "react-native";
import type { PrinterType } from "@/edysonpos/services/printer-settings";
...
export interface SalesOverviewReportData {
  dateLabel: string;
  /** The date the report covers (e.g. selected date when filtering "today") */
  reportDate?: Date;
  printedAt?: Date;
  printedBy?: string;
  kontanTotal: number;
  ...
  /** Cashier name -> unpaid grosir amount (Grosir Belum Bayar) */
  grosirBelumBayarByCashier?: Record<string, number>;
}
...
class ThermalPrinterService {
  ...
  async printTestReceipt(): Promise<void> {
    ...
    const commands = ESCPOSCommands.combine(
      ...
      ESCPOSCommands.text("Date: " + new Date().toLocaleString() + "\n"),
  ...
  async printSalesOverviewReport(data: SalesOverviewReportData): Promise<void> {
    ...
    const commands = ESCPOSCommands.combine(
      ...
      ESCPOSCommands.text("TOKO EDYSON\n"),
      ESCPOSCommands.selectPrintMode({}),
      ESCPOSCommands.text("Laporan Penjualan\n"),
      ESCPOSCommands.text(`${data.dateLabel}\n`),
      ...(reportDateStr ? [ESCPOSCommands.text(`Periode: ${reportDateStr}\n`)] : []),
      ...
      // Sales by cashier (paid only); Grosir Belum Bayar shown separately per cashier
      ...((() => {
        const byCashier = data.salesByCashier || {};
        const grosirBelumBayar = data.grosirBelumBayarByCashier || {};
        const allCashiers = new Set([...Object.keys(byCashier), ...Object.keys(grosirBelumBayar)]);
        if (allCashiers.size === 0) return [];

        const sorted = [...allCashiers].sort((ca, cb) => {
          const paidA = Object.values(byCashier[ca] || {}).reduce((s, x) => s + x.total, 0);
          const paidB = Object.values(byCashier[cb] || {}).reduce((s, x) => s + x.total, 0);
          return paidB - paidA;
        });
        ...
        return [
          ...
          ESCPOSCommands.selectPrintMode({}),
          ...sorted.flatMap((cashier) => {
            const byPayment = byCashier[cashier] || {};
            const lines: any[] = [
            ...
            const unpaid = grosirBelumBayar[cashier];
            if (unpaid != null && unpaid > 0) {
              lines.push(ESCPOSCommands.text(`  grosir belum bayar IDR ${formatIDR(unpaid)}\n`));
            }
            return lines;
          }),
          ESCPOSCommands.text(dotted),
